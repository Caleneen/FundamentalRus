    ...
    if (tab === 'stage') {
        if (subtab === 'Structures') {
            ...
            if (active === 1) {
                ...
                getId('reset0Button').textContent = `Цель - ${format(calculateEffects.dischargeCost(), { padding: true })} Энергии`;
        ...
        if (subtab === 'Settings') {
            ...
            if (global.lastSave >= 1000) { getId('isSaved').textContent = `${format(global.lastSave / 1000, { type: 'time' })} назад`; }
