...
export const global: globalType = {
    ...
    buildingsInfo: {
        ...
        name: [
            [],
            ['Mass', 'Преонов', 'Кварков', 'Частиц', 'Атомов', 'Молекул'], //[0] Must be 'Mass'
            ['Молей', 'Каплей', 'Луж', 'Прудов', 'Озёр', 'Морей', 'Океанов'],
    ...
    upgradesInfo: [
        {} as globalType['upgradesInfo'][0], { //Stage 1
            ...
            effectText: [
                ...
                () => `Новая субстанция для аккреции, она даст буст космичкеской пыли, основываясь на количестве.\n(Формула: (купленные * текущие) ^${format(calculateEffects.S3Upgrade1_power())} | Буст: ${format(calculateEffects.S3Upgrade1(), { padding: true })})`,
    ...
    ASRInfo: {
        ...
        effectText: () => {
            ...
            return `Автоматически покупает ${unlocked ? names[index] : '(Неизвестно)'} (считается за купленные) если цена в ${format(player.toggles.shop.wait[stageIndex])} раз больше текущих ресурсов.\n(Разблокированные авто структуры: ${unlocks !== '' ? `${unlocks}${unlocks.includes('!') ? ". '!' значит что игнорирует значение ожидания" : ''}` : 'none'})`;
        },
        costRange: [
            [],
    ...
    challengesInfo: [{
        ...
        rewardText: [[
            ...
            ["'Повышение ранга' (аккреция)", 'New Abyss themed Strangeness', 'Work in progress', 'Work in progress']
        ], [
            [],
